...
import immortal.half.wu.DeviceManagerUtil;
...
public class DevicesStatueReader {

    private static DevicesStatueReader deviceListener;

    public static DevicesStatueReader getInstance() {
        if (deviceListener == null) {
            synchronized (DevicesStatueReader.class) {
                if (deviceListener == null) {
                    deviceListener = new DevicesStatueReader();
                }
            }
        }
    ...
    private IJobListener<String[]> readConnectDeviceListener = new IJobListener<String[]>() {
        ...
        public void onError(Exception e) {
            LogUtil.e(DeviceManagerUtil.TAG, "DevicesStatueReader : 设备状态轮询异常," + e.getMessage());
            startRead();
        }
        ...
        public void onComplete(String[] result) {
            ...
            if (connectChangeDevices.size() > 0) {
                LogUtil.i(DeviceManagerUtil.TAG, "DevicesStatueReader : 有设备断开：" + connectChangeDevices);
            }
            ...
            if (connectChangeDevices.size() > 0) {
                LogUtil.i(DeviceManagerUtil.TAG, "DevicesStatueReader : 有新的设备连接：" + connectChangeDevices);
            }
